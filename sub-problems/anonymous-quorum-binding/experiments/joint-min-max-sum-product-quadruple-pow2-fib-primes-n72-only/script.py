#!/usr/bin/env python3
"""
Continuation of 115: for pow2, Fibonacci, first-n primes only, test n=72 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

115 showed no collision for these three at n=71 (101+102+...+115 through n=71).

Exit:
- PASS: at least one schedule has a collision at n=72
- FAIL: no schedule collides at n=72
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n72", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n72", flush=True)
