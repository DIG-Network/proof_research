#!/usr/bin/env python3
"""
Continuation of 107: for pow2, Fibonacci, first-n primes only, test n=64 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

107 showed no collision for these three at n=63 (101+102+103+104+105+106+107 through n=63).

Exit:
- PASS: at least one schedule has a collision at n=64
- FAIL: no schedule collides at n=64
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n64", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n64", flush=True)
