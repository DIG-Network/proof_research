...
K(S) = (min w_i, max w_i, prod w_i) over i in S — exact integers.
K_M = (min, max, prod mod M).

Report cross-shell collisions for K and first M with collision for K_M.
Parent: min-max-sum-triple-weight-five-six-shell-collision (091).
"""
...
def triple_mmp(ws: list[int], subset: tuple[int, ...]) -> tuple[int, int, int]:
    vals = [ws[i] for i in subset]
    return (min(vals), max(vals), prod(vals))


def shell_map(ws: list[int], r: int) -> dict[tuple[int, int, int], tuple[int, ...]]:
    m: dict[tuple[int, int, int], tuple[int, ...]] = {}
    for comb in combinations(range(N), r):
        k = triple_mmp(ws, comb)
        m.setdefault(k, comb)
    return m
...
def main() -> None:
    ...
    for mod in range(2, 2000):
        ...
        if interm:
            ...
            break
    if first_m is None:
        print("no_mod_collision_M<2000", flush=True)
        print("INCONCLUSIVE", flush=True)
        sys.exit(2)

    if not inter:
        print("FAIL_no_exact_cross_shell", flush=True)
        sys.exit(1)
