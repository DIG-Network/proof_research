#!/usr/bin/env python3
"""
Continuation of 131: for pow2, Fibonacci, first-n primes only, test n=88 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

131 showed no collision for these three at n=87 (101+102+...+131 through n=87).

Exit:
- PASS: at least one schedule has a collision at n=88
- FAIL: no schedule collides at n=88
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n88", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n88", flush=True)
