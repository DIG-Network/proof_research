#!/usr/bin/env python3
"""
Continuation of 123: for pow2, Fibonacci, first-n primes only, test n=80 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

123 showed no collision for these three at n=79 (101+102+...+123 through n=79).

Exit:
- PASS: at least one schedule has a collision at n=80
- FAIL: no schedule collides at n=80
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n80", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n80", flush=True)
