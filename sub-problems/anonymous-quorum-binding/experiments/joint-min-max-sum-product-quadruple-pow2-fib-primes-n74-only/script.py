#!/usr/bin/env python3
"""
Continuation of 117: for pow2, Fibonacci, first-n primes only, test n=74 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

117 showed no collision for these three at n=73 (101+102+...+117 through n=73).

Exit:
- PASS: at least one schedule has a collision at n=74
- FAIL: no schedule collides at n=74
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n74", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n74", flush=True)
