#!/usr/bin/env python3
"""
Continuation of 108: for pow2, Fibonacci, first-n primes only, test n=65 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

108 showed no collision for these three at n=64 (101+102+...+108 through n=64).

Exit:
- PASS: at least one schedule has a collision at n=65
- FAIL: no schedule collides at n=65
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n65", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n65", flush=True)
