#!/usr/bin/env python3
"""
Continuation of 132: for pow2, Fibonacci, first-n primes only, test n=89 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

132 showed no collision for these three at n=88 (101+102+...+132 through n=88).

Exit:
- PASS: at least one schedule has a collision at n=89
- FAIL: no schedule collides at n=89
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n89", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n89", flush=True)
