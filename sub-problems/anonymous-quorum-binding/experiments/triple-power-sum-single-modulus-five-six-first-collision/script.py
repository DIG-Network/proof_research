#!/usr/bin/env python3
"""
n=10, w_i=i+1. p_k(S) = sum_{i in S} w_i^k for k=1,2,3.
...
def main() -> None:
    ...
    def powers(indices: tuple[int, ...]) -> tuple[int, int, int]:
        p1 = p2 = p3 = 0
        for i in indices:
            x = w[i]
            p1 += x
    ...
    for m in range(2, args.scan_max + 1):
        keys5: dict[tuple[int, int, int], tuple[int, ...]] = {}
        for t, pk in zip(five_t, five):
            keys5[(pk[0] % m, pk[1] % m, pk[2] % m)] = t
        for j, t6 in enumerate(six_t):
            pk = six[j]
            k = (pk[0] % m, pk[1] % m, pk[2] % m)
            if k in keys5:
                t5 = keys5[k]
                print(f"FIRST_COLLISION_M={m}")
                print(f"mod_key={k}")
                print(f"five_indices={t5} weights={tuple(w[i] for i in t5)} raw={powers(t5)}")
                print(f"six_indices={t6} weights={tuple(w[i] for i in t6)} raw={powers(t6)}")
                print("PASS")
                return
