#!/usr/bin/env python3
"""
Continuation of 126: for pow2, Fibonacci, first-n primes only, test n=83 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

126 showed no collision for these three at n=82 (101+102+...+126 through n=82).

Exit:
- PASS: at least one schedule has a collision at n=83
- FAIL: no schedule collides at n=83
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n83", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n83", flush=True)
