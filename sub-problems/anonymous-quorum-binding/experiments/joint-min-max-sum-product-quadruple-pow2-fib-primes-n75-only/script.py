#!/usr/bin/env python3
"""
Continuation of 118: for pow2, Fibonacci, first-n primes only, test n=75 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

118 showed no collision for these three at n=74 (101+102+...+118 through n=74).

Exit:
- PASS: at least one schedule has a collision at n=75
- FAIL: no schedule collides at n=75
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n75", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n75", flush=True)
