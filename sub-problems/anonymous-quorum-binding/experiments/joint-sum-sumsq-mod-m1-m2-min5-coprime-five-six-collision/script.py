...
Lex-first (M1, M2) with M1,M2 in [min_m, scan_max], gcd(M1,M2)==1:
  first collision on (sum w mod M1, sum w^2 mod M2).
Default min_m=5 (follow-up to min4-coprime experiment 057).
"""
...
def main() -> None:
    ...
    p.add_argument("--scan-max", type=int, default=250, help="max M1 and M2 inclusive")
    p.add_argument("--min-m", type=int, default=5, help="minimum M1 and M2")
    args = p.parse_args()
    cap = args.scan_max
    lo = args.min_m
            ...
            for idx in six:
                ...
                if key in by_key:
                    ...
                    if kind == "5":
                        ...
                        print(f"M1={m1} M2={m2} gcd={gcd(m1,m2)} min_m={lo} key={key}")
                        print(
    ...
    print(f"NO_COLLISION_COPRIME_min{lo}_IN_[{lo},{cap}]^2")
    print("INCONCLUSIVE")
