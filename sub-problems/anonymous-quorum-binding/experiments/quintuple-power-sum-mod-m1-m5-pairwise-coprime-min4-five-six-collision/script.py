...
Lex-first (M1,...,M5) with min_m <= Mi <= scan_max and pairwise gcd(Mi,Mj)==1:
  first collision on (p1 mod M1, ..., p5 mod M5).

Default min_m=4 (follow-up to 079 which used min_m=2, parallel to 060 vs 059).
"""
...
def main() -> None:
    ...
    p.add_argument(
        ...
        type=int,
        default=25,
        help="max each Mi inclusive (raise if INCONCLUSIVE; cost ~ O((cap-min)^5 * 462))",
    )
