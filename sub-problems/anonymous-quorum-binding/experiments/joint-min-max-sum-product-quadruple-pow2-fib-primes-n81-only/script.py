#!/usr/bin/env python3
"""
Continuation of 124: for pow2, Fibonacci, first-n primes only, test n=81 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

124 showed no collision for these three at n=80 (101+102+...+124 through n=80).

Exit:
- PASS: at least one schedule has a collision at n=81
- FAIL: no schedule collides at n=81
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n81", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n81", flush=True)
