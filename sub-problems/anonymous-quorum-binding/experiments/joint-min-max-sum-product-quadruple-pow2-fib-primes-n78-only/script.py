#!/usr/bin/env python3
"""
Continuation of 121: for pow2, Fibonacci, first-n primes only, test n=78 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

121 showed no collision for these three at n=77 (101+102+...+121 through n=77).

Exit:
- PASS: at least one schedule has a collision at n=78
- FAIL: no schedule collides at n=78
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n78", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n78", flush=True)
