#!/usr/bin/env python3
"""
Continuation of 122: for pow2, Fibonacci, first-n primes only, test n=79 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

122 showed no collision for these three at n=78 (101+102+...+122 through n=78).

Exit:
- PASS: at least one schedule has a collision at n=79
- FAIL: no schedule collides at n=79
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n79", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n79", flush=True)
