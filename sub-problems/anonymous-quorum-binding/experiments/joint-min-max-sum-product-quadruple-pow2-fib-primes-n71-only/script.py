#!/usr/bin/env python3
"""
Continuation of 114: for pow2, Fibonacci, first-n primes only, test n=71 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

114 showed no collision for these three at n=70 (101+102+...+114 through n=70).

Exit:
- PASS: at least one schedule has a collision at n=71
- FAIL: no schedule collides at n=71
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n71", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n71", flush=True)
