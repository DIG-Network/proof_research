#!/usr/bin/env python3
"""
For four 098 schedules (pow2, triangular, Fibonacci, first-n primes), scan n in [n_lo, n_hi]
and record the minimal n with a 5-vs-6 cross-shell collision for exact
K(S)=(min w, max w, sum w, product w).

100 scanned [11,35] with no collision for any schedule. This experiment extends n_hi to 45.

Memory: store only C(n,5) keys with one witness per key; scan C(n,6) and test membership
(equivalent to intersecting full 5- and 6-key sets, but avoids storing all 6-keys).

Exit:
- PASS: every schedule has at least one collision for some n in [n_lo, n_hi]
- FAIL: at least one schedule has no collision in the whole range
"""
...
def main() -> None:
    ...
    if all_collide:
        print("PASS_all_four_schedules_collide_by_n45", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_some_schedule_has_no_collision_through_n45", flush=True)
