#!/usr/bin/env python3
"""
Continuation of 129: for pow2, Fibonacci, first-n primes only, test n=86 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

129 showed no collision for these three at n=85 (101+102+...+129 through n=85).

Exit:
- PASS: at least one schedule has a collision at n=86
- FAIL: no schedule collides at n=86
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n86", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n86", flush=True)
