#!/usr/bin/env python3
"""
Continuation of 102: for pow2, Fibonacci, first-n primes only, scan n in [51,55]
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

102 showed no collision for these three for any n in [46,50] (and 101 through n=45).

Exit:
- PASS: at least one schedule has a collision for some n in [51,55]
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_in_n51_to_55", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_in_n51_to_55", flush=True)
