#!/usr/bin/env python3
"""
Continuation of 127: for pow2, Fibonacci, first-n primes only, test n=84 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

127 showed no collision for these three at n=83 (101+102+...+127 through n=83).

Exit:
- PASS: at least one schedule has a collision at n=84
- FAIL: no schedule collides at n=84
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n84", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n84", flush=True)
