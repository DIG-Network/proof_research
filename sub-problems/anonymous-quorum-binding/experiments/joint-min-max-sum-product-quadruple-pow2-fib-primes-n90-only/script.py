#!/usr/bin/env python3
"""
Continuation of 133: for pow2, Fibonacci, first-n primes only, test n=90 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

133 showed no collision for these three at n=89 (101+102+...+133 through n=89).

Exit:
- PASS: at least one schedule has a collision at n=90
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n90", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n90", flush=True)
