#!/usr/bin/env python3
"""
Continuation of 125: for pow2, Fibonacci, first-n primes only, test n=82 only
for 5-vs-6 cross-shell collision on exact K(S)=(min w, max w, sum w, product w).

125 showed no collision for these three at n=81 (101+102+...+125 through n=81).

Exit:
- PASS: at least one schedule has a collision at n=82
- FAIL: no schedule collides at n=82
"""
...
def main() -> None:
    ...
    if any_collide:
        print("PASS_at_least_one_schedule_collides_at_n82", flush=True)
        print("PASS", flush=True)
        return

    print("FAIL_no_schedule_collides_at_n82", flush=True)
