...
Exhaustive scan at n=7, shell {2,3}:
  ...
  coord + full r=2 XOR menu + three r=3 XOR splits (unordered triple of triple indices).

C(7,3)=35 triples => C(35,3)=6545 unordered triples.
"""
...
def main() -> None:
    ...
    witnesses: list[tuple[int, int, int, float]] = []
    t_wall0 = time.perf_counter()
    ncomb = 0
    for i, j, k in itertools.combinations(range(35), 3):
        ncomb += 1
        xor_lists = [p2, [p3[i], p3[j], p3[k]]]
        t0 = time.perf_counter()
        md, _ = mod.min_depth_for_language(masks, coord, xor_lists, n, LRU_CAP)
        dt = time.perf_counter() - t0
        if md == 2:
            witnesses.append((i, j, k, dt))
    t_wall = time.perf_counter() - t_wall0

    print(f"triples_checked={ncomb} lru_cap={LRU_CAP} wall_sec={t_wall:.3f}", flush=True)
    print(f"witness_min_d2_count={len(witnesses)}", flush=True)
    for row in witnesses[:40]:
        i, j, k, dt = row
        print(f"  witness i={i} j={j} k={k} dp_sec={dt:.4f}", flush=True)
    if len(witnesses) > 40:
