#!/usr/bin/env python3
r"""
n=8, shell {2,3,4}: exhaustive partial r=6 submenu (K=1 of 28 splits) — off-diagonal s012 structure scan.

Enumerates all 28 singleton index choices into the full r=6 XOR split list (len=28), appends those
...
def main() -> None:
    ...
    print(
        f"STRATUM_TOTAL={STRATUM_TOTAL} menus={len(menus)} k_p6={K_P6} "
    ...
    if found_proper:
        print(
            "PASS: at least one K=1 partial r=6 menu yields 0<stratum_min_d2<107800",
            flush=True,
        )
        sys.exit(0)
    print(
        "FAIL: every K=1 partial r=6 menu yields stratum_min_d2 in {0,107800} on this stratum",
        flush=True,
    )
    sys.exit(1)
