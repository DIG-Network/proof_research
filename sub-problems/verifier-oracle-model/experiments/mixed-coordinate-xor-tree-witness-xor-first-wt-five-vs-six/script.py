#!/usr/bin/env python3
"""
XOR-first witness for mixed coordinate + pair-XOR (same feasibility as 066 / 067).

exists_tree: unchanged from 066 (try coordinates, then pair-XOR).
witness_xor_first: try all pair-XOR splits (lex i<j) before any coordinate split,
still requiring exists_tree on both children.

Compare: mixed-coordinate-xor-tree-witness-wt-five-vs-six (coord-first witness, 067).
"""
...
@lru_cache(maxsize=None)
def witness_xor_first(S: tuple[int, ...], depth_remaining: int) -> tuple:
    if pure(S):
        if not S:
            return EMPTY_LEAF
        ...
        for j in range(i + 1, N):
            ...
            if not S0:
                if exists_tree(S1, depth_remaining - 1):
                    return ("X", i, j, EMPTY_LEAF, witness_xor_first(S1, depth_remaining - 1))
            elif not S1:
                if exists_tree(S0, depth_remaining - 1):
                    return ("X", i, j, witness_xor_first(S0, depth_remaining - 1), EMPTY_LEAF)
            else:
                if exists_tree(S0, depth_remaining - 1) and exists_tree(
                    ...
                    return (
                        ...
                        i,
                        j,
                        witness_xor_first(S0, depth_remaining - 1),
                        witness_xor_first(S1, depth_remaining - 1),
                    )
    ...
    for i in range(N):
        ...
        if not S0:
            if exists_tree(S1, depth_remaining - 1):
                return ("C", i, EMPTY_LEAF, witness_xor_first(S1, depth_remaining - 1))
        elif not S1:
            if exists_tree(S0, depth_remaining - 1):
                return ("C", i, witness_xor_first(S0, depth_remaining - 1), EMPTY_LEAF)
        else:
            if exists_tree(S0, depth_remaining - 1) and exists_tree(
                S1, depth_remaining - 1
            ):
    ...
    raise RuntimeError("witness_xor_first: no solvable split — exists_tree mismatch")
...
def count_internal_by_type(t: tuple) -> tuple[int, int]:
    """Return (num_coord, num_xor) internal nodes."""
    if t[0] == "L":
        return 0, 0
    if t[0] == "C":
        _, _, a, b = t
        c0, x0 = count_internal_by_type(a)
        c1, x1 = count_internal_by_type(b)
        return 1 + c0 + c1, x0 + x1
    if t[0] == "X":
        _, _, _, a, b = t
        c0, x0 = count_internal_by_type(a)
        c1, x1 = count_internal_by_type(b)
        return c0 + c1, 1 + x0 + x1
    raise ValueError(t)
