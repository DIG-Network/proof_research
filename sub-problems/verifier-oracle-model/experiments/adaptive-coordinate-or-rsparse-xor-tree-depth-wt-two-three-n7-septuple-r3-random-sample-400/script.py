#!/usr/bin/env python3
"""
n=7, shell {2,3}: random sample of unordered 7-tuples of r=3 XOR split indices.

Language: coord + full r=2 XOR menu + seven chosen r=3 XOR splits.

C(7,3)=35 triples => C(35,7)=6724520 unordered septuples; we sample SAMPLE_SEPTS
with RNG seed SEED for bounded wall time before a full exhaustive C(35,7) scan.
"""
...
def main() -> None:
    ...
    universe = list(itertools.combinations(range(35), 7))
    assert len(universe) == 6_724_520
    rng = random.Random(SEED)
    k = min(SAMPLE_SEPTS, len(universe))
    ...
    for idx, tup in enumerate(sampled, start=1):
        if idx % PROGRESS_EVERY == 0:
            print(f"progress septs_checked={idx}/{k}", flush=True)
        i, j, a, b, c, d, e = tup
        xor_lists = [p2, [p3[i], p3[j], p3[a], p3[b], p3[c], p3[d], p3[e]]]
    ...
    print(
        f"seed={SEED} sample_septs={k} total_universe=6724520 "
