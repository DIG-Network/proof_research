#!/usr/bin/env python3
"""
Among feasible XOR roots (070), build root+XOR + 067 witness on children; count ties at min SHA256(JSON).

Outcome:
  PASS + UNIQUE_MIN — hypothesis holds (exactly one minimizer)
  PASS + TIES_AT_MIN — uniqueness fails (>=2 roots share min hash); still valid script
"""
...
def main() -> None:
    ...
    min_h = rows[0][0]
    minimizers = [ij for h, ij in rows if h == min_h]
    print(f"min_sha256={min_h}")
    ...
    if min_h == h067:
        print("MIN_MATCHES_067_DEFAULT")
    else:
        print("MIN_DIFFERS_FROM_067_DEFAULT")
