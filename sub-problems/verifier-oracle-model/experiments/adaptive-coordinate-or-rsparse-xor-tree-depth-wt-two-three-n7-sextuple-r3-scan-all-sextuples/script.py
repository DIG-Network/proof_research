...
Exhaustive scan at n=7, shell {2,3}:
  ...
  coord + full r=2 XOR menu + six r=3 XOR splits (unordered sextuple of triple indices).

C(7,3)=35 triples => C(35,6)=1623160 unordered sextuples.
"""
...
def main() -> None:
    ...
    witnesses: list[tuple[int, int, int, int, int, int, float]] = []
    t_wall0 = time.perf_counter()
    ncomb = 0
    for i, j, k, ell, m, s in itertools.combinations(range(35), 6):
        ncomb += 1
        if ncomb % PROGRESS_EVERY == 0:
            print(f"progress sexts_checked={ncomb}", flush=True)
        xor_lists = [p2, [p3[i], p3[j], p3[k], p3[ell], p3[m], p3[s]]]
        t0 = time.perf_counter()
        md, _ = mod.min_depth_for_language(masks, coord, xor_lists, n, LRU_CAP)
        dt = time.perf_counter() - t0
        if md == 2:
            witnesses.append((i, j, k, ell, m, s, dt))
    t_wall = time.perf_counter() - t_wall0

    print(f"sexts_checked={ncomb} lru_cap={LRU_CAP} wall_sec={t_wall:.3f}", flush=True)
    print(f"witness_min_d2_count={len(witnesses)}", flush=True)
    for row in witnesses[:40]:
        i, j, k, ell, m, s, dt = row
        print(
            f"  witness i={i} j={j} k={k} ell={ell} m={m} s={s} dp_sec={dt:.4f}",
            flush=True,
        )
    if len(witnesses) > 40:
