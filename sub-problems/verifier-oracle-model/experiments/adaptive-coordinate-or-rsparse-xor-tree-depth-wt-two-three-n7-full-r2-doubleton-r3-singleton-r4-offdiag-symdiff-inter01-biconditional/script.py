#!/usr/bin/env python3
r"""
n=7, shell {2,3}: biconditional for off-diagonal |T_i∩T_j| in {0,1}.

Language: coord + full r=2 + two r=3 XOR splits (i<j only in analysis) + one r=4 XOR split.

Restrict to i<j with s = popc(T_i & T_j) in {0,1}. Hypothesis on this stratum:

  min_d == 2  <=>  Q_mask == (T_i XOR T_j).

For s=0, symdiff has 6 bits, so Q (4-set) can never match — expect zero min_d=2 on s=0.

Exit 0 = PASS (biconditional holds on the s in {0,1} stratum).
...
def main() -> None:
    ...
    viol_d2_not_symdiff: list[str] = []
    viol_symdiff_but_not_d2: list[str] = []
    viol_d2_s0: list[str] = []

    count_cells_total = 0
    count_stratum = 0  # i<j and s in {0,1}
    count_stratum_s0 = 0
    count_stratum_s1 = 0
    count_stratum_d2 = 0
    count_stratum_pred = 0  # Q == symdiff
        ...
        for j in range(i, len(p3)):
            ...
            for k in range(len(p4)):
                ...
                if md == 2:
                    ...
                    if s_inter == 0:
                        viol_d2_s0.append(
                            f"i={i} j={j} k={k} md=2 unexpected s=0 "
                            f"T_i={triples[i]} T_j={triples[j]} Q={quads[k]}"
                        )
                    elif q_mask != d:
                        viol_d2_not_symdiff.append(
                            f"i={i} j={j} k={k} md=2 s=1 but Q!=symdiff "
                            f"T_i={triples[i]} T_j={triples[j]} Q={quads[k]} "
                            f"Dbits={d:07b} Qbits={q_mask:07b}"
                        )
                elif pred and md is not None and md != 2:
                    viol_symdiff_but_not_d2.append(
                        f"i={i} j={j} k={k} md={md} s={s_inter} pred_symdiff "
                        f"T_i={triples[i]} T_j={triples[j]} Q={quads[k]}"
                    )
    ...
    all_viol = viol_d2_s0 + viol_d2_not_symdiff + viol_symdiff_but_not_d2
    print(
        f"viol_counts d2_s0={len(viol_d2_s0)} d2_not_symdiff={len(viol_d2_not_symdiff)} "
        f"symdiff_not_d2={len(viol_symdiff_but_not_d2)} total={len(all_viol)}",
        flush=True,
    )
    if all_viol:
        for v in viol_d2_s0[:15]:
            print(f"VIOL_D2_S0: {v}", flush=True)
        for v in viol_d2_not_symdiff[:15]:
            print(f"VIOL_D2_NOT_SD: {v}", flush=True)
        for v in viol_symdiff_but_not_d2[:15]:
            print(f"VIOL_SD_NOT_D2: {v}", flush=True)
        extra = len(all_viol) - 45
        if extra > 0:
            print(f"... and {extra} more violations", flush=True)
        print(
            "FAIL: symdiff biconditional broken on off-diagonal |∩| in {0,1}",
            flush=True,
        )
        sys.exit(1)

    print(
        "PASS: for i<j and |T_i∩T_j| in {0,1}, min_d=2 iff Q equals symdiff(T_i,T_j)",
        flush=True,
    )
    sys.exit(0)
