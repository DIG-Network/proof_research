#!/usr/bin/env python3
"""
n=14 {7,8}: d=3-only, non-contiguous 1001-wide XOR submenu via indices 1,3,...,2001.

Complementary coset to the sibling experiment using 0,2,...,2000 (same |menu|=1001).
...
Sibling: ...-xor-odd-indices-1001-each-12e7-lru-8m (even-start interleave; misnamed ODD in that folder)
"""
...
TOTAL_XOR = 2002
# Complementary 1001 indices: odd starting at 1 → 1,3,5,...,2001
MENU_INDICES = list(range(1, TOTAL_XOR, 2))
assert len(MENU_INDICES) == 1001

RUNS: list[tuple[int, str]] = [
    (5, "r5_step2_start1_1001"),
    (9, "r9_step2_start1_1001"),
]
...
def run_r(r: int, label: str) -> tuple[int, str]:
    ...
    indices_arg = ",".join(str(i) for i in MENU_INDICES)
    cmd = [
        sys.executable,
        str(PARENT),
        "--skip-baseline",
    ...
    print(
        f"\n=== {label} r={r} |indices|={len(MENU_INDICES)} (1,3,…,2001) ===",
        flush=True,
    )
...
def main() -> None:
    ...
    if witness:
        print(
            "PASS: at least one r-run witnessed d=3 feasible=True on 1,3,…,2001 menu",
            flush=True,
        )
        sys.exit(0)
    if any_error:
