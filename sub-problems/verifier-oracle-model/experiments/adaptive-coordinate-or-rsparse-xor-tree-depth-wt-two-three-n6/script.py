#!/usr/bin/env python3
"""
n=6, popcount in {2,3} (C(6,2)+C(6,3)=35 masks).

Same DP as n=7 {2,3} sweep: coord + r-sparse XOR r=2..(n-1), unions, full n-XOR.
Threshold slice aligned with n=7 driver: wt 2 vs wt 3.
"""
    ...
    if args.union_rs is not None:
        ...
        for r in rs:
            ...
            if r == 3 and args.union_r3_indices is not None:
                idxs = [
                    int(x.strip())
                    for x in args.union_r3_indices.split(",")
                    if x.strip()
                ]
                n3 = math.comb(N, 3)
                for i in idxs:
