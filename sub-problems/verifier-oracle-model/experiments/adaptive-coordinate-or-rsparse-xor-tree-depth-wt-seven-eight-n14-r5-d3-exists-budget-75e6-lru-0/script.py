#!/usr/bin/env python3
"""
n=14 {7,8}: r=5, d=3-only with --max-exists-calls 75_000_000, --lru-maxsize 0 (unbounded memo).

After 75e6/12M LRU SIGKILL on this host class, session-state: retry same visit budget
without LRU eviction (unbounded lru_cache) + explicit wall timeout on large RAM.
