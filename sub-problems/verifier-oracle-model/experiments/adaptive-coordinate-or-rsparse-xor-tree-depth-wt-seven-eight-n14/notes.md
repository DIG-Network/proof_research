...
- **`r=9`:** **4M LRU** looked like **cache thrash** (high CPU, ~18+ min on **`d=3`**) — killed earlier. **Follow-up** on **~15 GiB** with **`--lru-maxsize 0`:** **`timeout 1200`** still **stuck on `d=3`** (no completion) — confirms **hard subproblem**, not only **LRU eviction**.
