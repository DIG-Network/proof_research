...
Exhaustive scan at n=7, shell {2,3}:
  ...
  coord + full r=2 XOR menu + seven r=3 XOR splits (unordered septuple of triple indices).

C(7,3)=35 triples => C(35,7)=6724520 unordered septuples.

Stops at the first min_d==2 witness (PASS). If none, FAIL after full enumeration.
"""
...
def main() -> None:
    ...
    witnesses: list[tuple[int, int, int, int, int, int, int, float]] = []
    t_wall0 = time.perf_counter()
    ncomb = 0
    for i, j, k, ell, m, s, t in itertools.combinations(range(35), 7):
        ncomb += 1
        if ncomb % PROGRESS_EVERY == 0:
            print(f"progress septs_checked={ncomb}", flush=True)
        xor_lists = [p2, [p3[i], p3[j], p3[k], p3[ell], p3[m], p3[s], p3[t]]]
        t0 = time.perf_counter()
        md, _ = mod.min_depth_for_language(masks, coord, xor_lists, n, LRU_CAP)
        dt = time.perf_counter() - t0
        if md == 2:
            witnesses.append((i, j, k, ell, m, s, t, dt))
            break
    t_wall = time.perf_counter() - t_wall0

    print(f"septs_checked={ncomb} lru_cap={LRU_CAP} wall_sec={t_wall:.3f}", flush=True)
    print(f"witness_min_d2_count={len(witnesses)}", flush=True)
    for row in witnesses[:40]:
        i, j, k, ell, m, s, t, dt = row
        print(
            f"  witness i={i} j={j} k={k} ell={ell} m={m} s={s} t={t} dp_sec={dt:.4f}",
            flush=True,
        )
    if len(witnesses) > 40:
