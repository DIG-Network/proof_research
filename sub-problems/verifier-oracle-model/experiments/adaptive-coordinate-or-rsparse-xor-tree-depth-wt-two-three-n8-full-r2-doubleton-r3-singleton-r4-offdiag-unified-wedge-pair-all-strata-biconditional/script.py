#!/usr/bin/env python3
r"""
n=8, shell {2,3,4}: biconditional on all off-diagonal strata s=|T_i∩T_j| in {0,1,2}
...
For i<j with s = popc(T_i & T_j) in {0,1,2}:
  ...
  W_ij = (T_i \ T_j) ∪ ([8] \ (T_i ∪ T_j))
  W_ji = (T_j \ T_i) ∪ ([8] \ (T_i ∪ T_j))
...
# n_triples=56, n_quads=70 → full grid 56*57/2*70 = 111_720 cells
...
def main() -> None:
    ...
    assert len(triples) == 56 and len(quads) == 70
