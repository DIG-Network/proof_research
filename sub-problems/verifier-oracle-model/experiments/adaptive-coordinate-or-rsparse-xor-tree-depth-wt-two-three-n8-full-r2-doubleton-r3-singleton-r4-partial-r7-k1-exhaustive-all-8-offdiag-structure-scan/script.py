#!/usr/bin/env python3
r"""
n=8, shell {2,3,4}: exhaustive partial r=7 submenu (K=1 of 8 splits) — off-diagonal s012 structure scan.

Enumerates all 8 singleton index choices into the full r=7 XOR split list (len=8), appends those
...
def main() -> None:
    ...
    if found_proper:
        print(
            "PASS: at least one K=1 partial r=7 menu yields 0<stratum_min_d2<107800",
            flush=True,
        )
        sys.exit(0)
    print(
        "FAIL: every K=1 partial r=7 menu yields stratum_min_d2 in {0,107800} on this stratum",
        flush=True,
    )
    sys.exit(1)
