#!/usr/bin/env python3
r"""
n=8, shell {2,3,4}: exhaustive partial r=5 submenu (K=2 of 56 splits) — off-diagonal s012 structure scan.

Enumerates all C(56,2)=1540 unordered pairs of indices into the full r=5 XOR split list, appends those
...
def _pool_init() -> None:
    global _G_MOD, _G_MASKS, _G_COORD, _G_P2, _G_P3, _G_P4, _G_P5
    _G_MOD = _load_parent()
    ...
    _G_P5 = _G_MOD.build_r_xor_partition_masks(_G_MASKS, 5)
...
def main() -> None:
    ...
    max_menus_env = os.environ.get("MAX_MENUS", "").strip()
    ...
    print(
        f"STRATUM_TOTAL={STRATUM_TOTAL} menus={len(menus)} k_p5={K_P5} workers={workers} "
    ...
    if found_proper:
        print(
            "PASS: at least one K=2 partial r=5 menu yields 0<stratum_min_d2<107800",
            flush=True,
        )
        sys.exit(0)
    print(
        "FAIL: every K=2 partial r=5 menu yields stratum_min_d2 in {0,107800} on this stratum",
        flush=True,
    )
    sys.exit(1)
