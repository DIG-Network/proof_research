#!/usr/bin/env python3
r"""
n=8, shell {2,3,4}: random partial r=6 submenu (K=2 of 28 splits) — off-diagonal s012 structure scan.

Base language: coord + full r=2 + doubleton r=3 + singleton r=4 (same grid as partial r5/r6/r7 scans).

For each of NUM_TRIALS independent RNG draws (SEED), sample K_P6 distinct indices into the full
r=6 XOR split list (len=28), append those splits only, and aggregate on the s∈{0,1,2} stratum:
...
def main() -> None:
    ...
    universe = list(range(len(p6)))

    print(
        "trial p6_indices stratum_min_d2 stratum_pred viol_d2_not_pred viol_pred_not_d2 wall_sec",
        flush=True,
    )
    found_proper = False
    total_wall = 0.0
    ...
    for t in range(1, NUM_TRIALS + 1):
        idx_tuple = tuple(sorted(rng.sample(universe, K_P6)))
    ...
    print(
        f"STRATUM_TOTAL={STRATUM_TOTAL} trials={NUM_TRIALS} k_p6={K_P6} seed={SEED} "
    ...
    if found_proper:
        print(
            "PASS: at least one partial r=6 K=2 draw yields 0<stratum_min_d2<107800",
            flush=True,
        )
        sys.exit(0)
    print(
