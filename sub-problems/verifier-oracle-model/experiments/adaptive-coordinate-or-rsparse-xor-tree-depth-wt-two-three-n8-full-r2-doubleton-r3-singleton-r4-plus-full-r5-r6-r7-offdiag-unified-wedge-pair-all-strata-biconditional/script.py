#!/usr/bin/env python3
r"""
n=8, shell {2,3,4}: same off-diagonal stratum s=|T_i∩T_j| in {0,1,2} as the
sparse-menu wedge experiment, but the XOR language is enriched by **full**
r=5, r=6, and r=7 XOR split menus (in addition to full r=2 + the chosen doubleton
r=3 + singleton r=4).

Motivation: the sparse menu alone yields **no** min_d=2 witnesses on this
stratum (vacuous biconditional). The full union r∈{2..7} on the {2,3} weight
shell has min_d=2; this experiment tests whether appending only the higher
arities restores a nonempty depth-2 stratum and whether the wedge law returns.

Exit 0 = PASS (biconditional holds, including vacuously); exit 1 = FAIL.
"""
...
def main() -> None:
    ...
    for i in range(len(p3)):
        for j in range(i, len(p3)):
            for k in range(len(p4)):
                xor_lists = [p2, [p3[i], p3[j]], [p4[k]], p5, p6, p7]
                ...
                if md == 2:
                    ...
                    if not pred:
                        viol_d2_not_pred.append(
                            ...
                            f"Wij={w_ij:08b} Wji={w_ji:08b} Qbits={q_mask:08b}"
                        )
                ...
                if count_cells_total % 8000 == 0:
                    print(
                        f"progress {count_cells_total}/111720 i={i} j={j} k={k} md={md}",
                        flush=True,
                    )
    ...
    if all_viol:
        ...
        print(
            "FAIL: unified W_ij∨W_ji biconditional broken (enriched r5–r7 menus)",
            flush=True,
        )
        sys.exit(1)
    ...
    print(
        ...
        "(language includes full r=5,6,7 XOR menus)",
        flush=True,
    )
    sys.exit(0)
