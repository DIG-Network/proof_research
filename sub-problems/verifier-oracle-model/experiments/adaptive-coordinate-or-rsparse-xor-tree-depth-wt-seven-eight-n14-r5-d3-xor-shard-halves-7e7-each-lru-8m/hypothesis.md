...
**Claim:** On `n=14`, shells `{7,8}`, `coord + r=5` XOR (2002 splits), `d=3`-only probe: splitting the XOR partition list into contiguous halves `[0:1001)` and `[1001:2002)` and running the bounded DP **sequentially** on each half with **7×10⁷** `exists_tree` budget and **8M** LRU (same caps as successful full-menu `r=5` 7e7 runs, but **half** the XOR menu per process) may yield **`d=3 feasible=True`** on at least one half—a **witness** that the full language admits depth ≤3 (superset monotonicity in parent `script.py`).
...
4. **Transfer candidate:** Reuse **`--xor-index-range`** on parent; **8M LRU** avoids 10M OOM on memory-bounded hosts; **7e7** matches prior **`r=5`** full-menu scale (known PARTIAL) but applied **per half** to test whether a contiguous half is already a witness menu.
...
**Builds on:** `adaptive-coordinate-or-rsparse-xor-tree-depth-wt-seven-eight-n14-r5-d3-exists-budget-7e7-lru-8m` (full menu PARTIAL); `adaptive-coordinate-or-rsparse-xor-tree-depth-wt-seven-eight-n14-r9-d3-xor-shard-halves-6e7-each-lru-10m` (wrapper pattern for `r=9`).
