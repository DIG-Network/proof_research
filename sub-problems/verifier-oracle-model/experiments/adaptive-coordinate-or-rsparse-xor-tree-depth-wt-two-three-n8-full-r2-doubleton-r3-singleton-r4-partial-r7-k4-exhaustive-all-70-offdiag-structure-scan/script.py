#!/usr/bin/env python3
r"""
n=8, shell {2,3,4}: exhaustive partial r=7 submenu (K=4 of 8 splits) — off-diagonal s012 structure scan.

Enumerates all C(8,4)=70 index quadruples into the full r=7 XOR split list (len=8), appends those
...
def main() -> None:
    ...
    menus = list(combinations(range(len(p7)), K_P7))
    assert len(menus) == 70

    print(
        "idx p7_indices stratum_min_d2 stratum_pred viol_d2_not_pred viol_pred_not_d2 wall_sec",
        flush=True,
    )
    found_proper = False
    total_wall = 0.0
    ...
    for idx, idx_tuple in enumerate(menus, start=1):
        ...
        if idx <= 5 or idx % 10 == 0 or idx == len(menus):
            print(
    ...
    print(
        f"STRATUM_TOTAL={STRATUM_TOTAL} menus={len(menus)} k_p7={K_P7} "
    ...
    if found_proper:
        print(
            "PASS: at least one K=4 partial r=7 menu yields 0<stratum_min_d2<107800",
            flush=True,
        )
        sys.exit(0)
    print(
        "FAIL: every K=4 partial r=7 menu yields stratum_min_d2 in {0,107800} on this stratum",
        flush=True,
    )
    sys.exit(1)
