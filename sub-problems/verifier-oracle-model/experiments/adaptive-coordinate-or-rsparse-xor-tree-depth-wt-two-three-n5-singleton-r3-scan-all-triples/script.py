#!/usr/bin/env python3
"""
n=5, shell {2,3}: full r=2 menu + each singleton r=3 split (10 scans).
Tests whether ONLY r3 index 0 yields min_d=2 (hypothesis from prior witness).

Parent: adaptive-coordinate-or-rsparse-xor-tree-depth-wt-two-n5/script.py

Exit: 0 = PASS (only index 0 has min_d=2 among singletons), 1 = FAIL.
"""
...
def main() -> None:
    ...
    # Pair-only: --union-rs 2 (no triple-XOR splits).
    cmd_pair = [
        sys.executable,
        str(PARENT),
        "--shells",
    ...
    for i in range(N_TRIPLES):
        ...
        print(
            f"r3_index={i} triple={trip} total_splits=11 min_d={md}",
            flush=True,
        )
        if md == 2:
            witnesses.append(i)
    ...
    if witnesses == [0]:
        print(
            "PASS (only singleton index 0 achieves min_d=2 with full r=2 fixed)",
            flush=True,
        )
        raise SystemExit(0)
    print(
        "FAIL (hypothesis falsified: witness set is not exactly {0})",
        flush=True,
    )
    raise SystemExit(1)
